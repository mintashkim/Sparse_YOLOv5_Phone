...
OrtDetector::OrtDetector(const std::string& modelPath,
...
void OrtDetector::getBestClassInfo(std::vector<float>::iterator it, const int& numClasses,
...
void OrtDetector::preprocessing(cv::Mat &image, float*& blob, std::vector<int64_t>& inputTensorShape)
{
...
std::vector<Detection> OrtDetector::postprocessing(const cv::Size& resizedImageShape,
...
std::vector<Detection> OrtDetector::detect(cv::Mat &image, const float& confThreshold = 0.4,
